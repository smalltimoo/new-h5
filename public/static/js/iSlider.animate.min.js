    ...
    iSlider && iSlider.extend(iSlider._animateFuncs, {
        ...
        depth: (function () {
            ...
            function depth(dom, axis, scale, i, offset, direct) {
                ...
                offset = i*35;
                if(i==2)  offset +=17
                // console.info(dom, axis, scale, i, offset, direct)

                var zoomScale = (4 - Math.abs(i - 1)) * 0.18+0.28;
                scale = dom.clientWidth;
                var outer = dom.parentElement;
                iSlider.setStyle(outer, 'perspective', scale * 4);
                dom.style.zIndex = i === 1 ? 1 : 0;
                let transX = (offset + 1 *( scale * (i - 1)))
                // console.info(transX)
                iSlider.setStyle(dom, 'transform', 'scale(' + zoomScale + ') translateZ(0) translate' + axis + '('+ transX + 'px)');
            }
            // function depth(dom, axis, scale, i, offset, direct) {
        ...
        flow: (function () {
            function flow(dom, axis, scale, i, offset, direct) {
                ...
                var outer = dom.parentElement;
                let domClientWidth =  dom.clientWidth||326;
                iSlider.setStyle(outer, 'perspective', scale * 4);
                offset = i === 1?(scale - domClientWidth)/2:-(i-1)*(scale - domClientWidth)*1.25;
                if(i == 2) offset = -(i-1)*(scale - domClientWidth)*0.25;
                if (i === 1) {
