<template>
  <div class="login">
    <div class="login_top">
      ...
      <span>
        <van-icon name="close" @click="$router.push({name:'Home'})" />
      </span>
      <span @click="showVan = true">
        账号密码登录
        <van-icon name="arrow" />
      </span>
    </div>
    ...
    <div class="container">
      <section class="login_main">
        <div class="logo">
          <img src="../assets/images/reglogin/logo.png" alt />
        </div>
        <p class="welcome">
          欢迎来到
          <font>{{name}}</font>
        ...
        <div style="position: relative;margin-top:25px;">
          ...
          <el-select v-model="areaCode" filterable placeholder="请选择" class="eye area">
            <el-option
              v-for="(item,index) in phoneAreaCode"
              :key="index"
          ...
          <input
            ...
            @click="$refs.phone1.close()"
            v-model="phone"
          />
          <cube-tip ref="phone1" class="tip" direction="top">{{$t('login.enterTruePhoneNumber')}}</cube-tip>
        </div>
        <div style="position: relative">
          ...
          <input
            ...
            style="width:100%"
            class="login-input"
            :placeholder="$t('login.enterVerifyCode')"
            @click="$refs.phone2.close()"
            v-model="verifyCode"
            autocomplete="off"
          />
          <div class="bg-code" @click="getCode">{{$t('login.getVerifyCode')}}</div>
          <cube-tip ref="phone2" class="tip" direction="top">{{$t('login.enterVerifyCode')}}</cube-tip>
        </div>
        <div style="position: relative">
          ...
          <input
            type="number"
            style="width:100%"
            class="login-input invit"
            maxlength="6"
            :placeholder="$t('login.enterInvitCode')"
            v-model="vm.invitationCode"
          />
          <cube-tip ref="reg9" class="tip" direction="top">{{ this.$t('login.enterTrueInvitCode') }}</cube-tip>
        </div>
        ...
        <input
          type="button"
          class="btn-login"
          style="background-color:#fff;color:#3d7eff;font-size:13px;margin-top:5px;"
          :value="'选择线路'"
          @click="isshowLines =true"
        />
      </section>
          ...
          <section class="login_main">
            ...
            <div style="margin-top:25px;">
              ...
              <input
                ...
                v-model="username"
              />
              <cube-tip ref="tip1" class="tip" direction="top">{{ this.$t('login.alert1') }}</cube-tip>
            </div>
            <div>
              ...
              <input
                ...
                maxlength="12"
                class="login-input"
                :placeholder="$t('login.mustPwd620')"
                @click="$refs.tip2.close()"
                v-model="password"
              />
              <cube-tip ref="tip2" class="tip" direction="top">{{ this.$t('login.alert1') }}</cube-tip>
            </div>
            <div style="position: relative">
              <h3 class="title">验证码</h3>
              <input
              ...
              <cube-tip ref="tip3" class="tip" direction="top">{{ this.$t('login.alert3') }}</cube-tip>
            </div>
            <div class="login-check">
    ...
    <line-selectdlg :panelShow="isshowLines" @close="CB_dialog"></line-selectdlg>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    mPhoneLogin() {
      ...
      this.mLoading(true);
      this.$http
        .post("/memberUser/quickLogin.json", {
          phoneNo: this.phone,
    ...
    getCode() {
      if (this.getCodes) {
        ...
        this.$http
          .post("/memberUser/sendVerifyCode.json", {
            mobile: this.phone,
            areaCode: this.areaCode
          })
          .then(result => {
