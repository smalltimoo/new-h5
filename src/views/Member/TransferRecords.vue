    ...
    <div class="mui-off-canvas-wrap mui-scalable">
      ...
      <div class="mui-inner-wrap">
          ...
          <div class="mui-scroll-wrapper">
            <div class="mui-scroll">
              ...
              <section class="el-container is-vertical">
                <Divider style="color:#fff">{{$t('member.transferRecords.tfr1') }}</Divider><!-- 资金类型-->
                <div class="mui-card">
                  <ul class="mui-table-view" style="max-height:300px;overflow-y:auto;">
                    <li class="mui-table-view-cell mui-radio mui-right" v-for="(item,i) in initData.actionTypes" :key="i">
                      <input name="radio" type="radio" :checked="item.checked" :value="item.id" v-model="searchVM.transType" >
                      {{item.value}}
                    </li>
                  </ul>
                </div>
        ...
        <header ref="TopHeader" class="mui-bar mui-bar-nav">
          ...
          <h1 class="mui-title">{{$t('member.transferRecords.tfr3') }}</h1><!--转账记录 -->
          <button class="mui-btn mui-btn-blue mui-btn-link mui-pull-right" @tap="mShowSearch">{{$t('member.transferRecords.tfr4') }}</button><!--筛选 -->
        </header>
        <!-- 主页面内容容器 -->
        <div class="mui-content mui-scroll-wrapper">
          <div class="mui-scroll">
            ...
            <div
              ...
              class="text-center"
            >{{$t('member.transferRecords.tfr5') }}</div><!--暂无数据 -->
            <Scroll v-if="dataList&&dataList.length>0&&!showSearch" ref="ivuScrollContainer" :on-reach-bottom="mReachBottom" :height="ivuScrollContainerHeight">
              <div class="rowbg el-row" v-for="(item,i) in dataList" :key="i">
                ...
                <div class="el-col el-col-6">
                  ...
                  <span class="Save"> {{item.transTypeStr}}</span>
                  </div>
                </div>
                ...
                <div class="el-col el-col-8">
                <div class="Types">
                  {{$t('member.transferRecords.tfr6') }}
                </div><!--金额 -->
                <div class="status">
                  {{item.transferAmount / 100}}
                </div>
                </div>
                <div class="el-col el-col-8">
                <div class="Types">
                  {{$t('member.transferRecords.tfr7') }}
                </div><!--转前金额 -->
                <div class="status">
                  {{item.beforeTransfer / 100}}
                </div>
                </div>
                <div class="el-col el-col-8">
                <div class="Types">
                  {{$t('member.transferRecords.tfr8') }}
                </div><!--转后金额 -->
                <div class="status">
                  {{item.afterTransfer / 100}}
                </div>
                </div>
              </div>
            </Scroll>
    ...
    mReachBottom() {
      return new Promise(resolve => {
        ...
        if (
          ...
          this.mLoading(false);
          //没有更多记录了
          this.mAlert(this.$t('member.transferRecords.tfr11'));
          resolve();
        }
      });
    },
