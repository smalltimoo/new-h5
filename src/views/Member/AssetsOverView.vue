    ...
    <div class="container">
        ...
        <div class="wallet">
          ...
          <div v-for="item in walletlist" :key="item.gameCompanyId" class="wallet-line">
            <div style="display: flex;align-items: center">
              ...
              <span
                v-text="item.gameCompanyName"
                style="padding-left: 15px;text-align: left; line-height: 20px;"
              ></span>
            </div>
            <span
            ...
            >
              ...
              >{{$t('member.assetsOverView.ao6')}}</span>
              <img
                v-if="!item.loading"
      ...
      <div class="bottom">
        <span style="background-color: grey">
          <router-link :to="{name:'Withdrawals'}">{{$t('member.assetsOverView.ao8')}}</router-link>
        </span>
        <span>
          <router-link :to="{name:'OnlineDeposit'}">{{$t('member.assetsOverView.ao7')}}</router-link>
        </span>
      </div>
    </div>
  </div>
  ...
  methods: {
    getcoins(a) {
      if (a == 0) {
        ...
        return this.walletlist[0].coin / 100 || 0.0;
      }
    },
    mGetCoin() {
      this.mLoading(true);
      this.$http
        ...
        .then(result => {
          ...
          this.otherCoins = parseFloat(
            (this.totalCoins - this.walletlist[0].coin) / 100
          ).toFixed(2);
          this.chartData.rows = [
    ...
    mGetError(gameCompanyId) {
      ...
      this.$http
        .post("/managerGame/getMemberGameBlance.json", {
    ...
    transferAll() {
      ...
      let index = 0;
      if (
        this.walletlist.filter(item => item.gameCompanyId != 0 && item.coin > 0)
          .length == 0
      ) {
