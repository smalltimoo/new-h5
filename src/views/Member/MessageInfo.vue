    ...
    <div class="publicpage">
      <div class="userSettings publicpage_view">
        <div class="TopHeader">
          ...
          </div>{{ $t('member.messageInfo.mi1') }}
        </div><!--信息内容 -->
        <div class="G_AboutOne ount_view">
          <h1 class="G_AboutOne_title" style="color:yellow">
            <Icon type="ios-radio-button-on" style="margin-right:10px;"/>
            {{ $t('member.messageInfo.mi2') }}
          </h1><!--标题 -->
            <div style="font-size:22px; text-align:center;">{{vm.msgTitle}}</div>
          <h1 class="G_AboutOne_title" style="color:yellow">
            <Icon type="ios-radio-button-on" style="margin-right:10px;"/> {{ $t('member.messageInfo.mi1') }}
          </h1><!--信息内容 -->
          <h4 class="G_AboutOne_text" >{{vm.msgInfo}}</h4>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    mInit() {
      this.$http
        .post("/memberUser/membermsginfo.json", { id: this.id })
        .then(result => {
          if (result.code == 0) {
            this.vm = Object.assign({}, result.data);
          }
        });
    }
  }
